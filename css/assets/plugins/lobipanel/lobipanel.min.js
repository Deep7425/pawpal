(Math.randomString = function (n) {
        ...
        return (
            ...
            object.children().each(function (index, el) {
                ...
                i > oldIndex && index > oldIndex && i >= index
                    ...
                    : oldIndex >= i && index > i && oldIndex >= index && $el.attr("data-index", parseInt($el.attr("data-index"), 10) + 1);
            }),
            this
        );
    }),
    ...
    $(function () {
        var LobiPanel = function ($el, options) {
            ...
            var $heading,
                ...
                _getMaxZIndex = function () {
                    var style,
                        max,
                        cur,
                        panels = $(".lobipanel.panel-unpin:not(.panel-minimized.panel-expanded)");
                    if (0 === panels.length) return { id: "", "z-index": LobiPanel.PRIVATE_OPTIONS.initialZIndex };
                ...
                _adjustForScreenSize = function () {
                    me.disableTooltips(),
                        ...
                        me.isOnFullScreen() && $body.css({ width: _calculateBodyWidth(me.$el.width()), height: _calculateBodyHeight(me.$el.height()) });
                },
                ...
                _setBodyHeight = function () {
                    "auto" !== me.$options.bodyHeight && $body.css({ height: me.$options.bodyHeight, overflow: "auto" });
                },
            ...
            (this.isPanelInit = function () {
                ...
                (this.unpin = function () {
                    ...
                    me.$el.css({ position: "fixed", width: panelWidth, height: panelHeight });
                    var bHeight = _calculateBodyHeight(panelHeight),
                        bWidth = _calculateBodyWidth(panelWidth);
                    return $body.css({ width: bWidth, height: bHeight }), me.$options.draggable && me.enableDrag(), "none" !== me.$options.resize && me.enableResize(), _saveState("unpinned"), _triggerEvent("onUnpin"), me;
                }),
                        ...
                        me.$el.hasClass("panel-expanded") || me.$el.attr("old-style", me.$el.attr("style")),
                            me.$el.animate({ left: left, top: top, width: 200, height: footer.height() }, 100, function () {
                                me.$el.hasClass("panel-expanded") &&
                                    ...
                                    me.$el
                                        .find(".panel-heading [data-func=expand] ." + LobiPanel.PRIVATE_OPTIONS.iconClass)
                ...
                (this.toFullScreen = function () {
                    ...
                    if (me.isPinned() || me.isMinimized()) {
                        me.enableTooltips(),
                            me.$el.css({ position: "fixed", "z-index": res["z-index"] + 1, left: me.$el.offset().left, top: me.$el.offset().top - $(window).scrollTop(), width: me.$el.width(), height: me.$el.height() }),
                            $("body").append(me.$el);
                        var footer = _getFooterForMinimizedPanels();
                    ...
                    return (
                        me.$el.animate({ width: $(window).width(), height: $(window).height() - toolbarHeight, left: 0, top: 0 }, me.$options.expandAnimation, function () {
                            me.$el.css({ width: "", height: "", right: 0, bottom: toolbarHeight }),
                                ...
                                $body.css({ width: _calculateBodyWidth(me.$el.width()), height: _calculateBodyHeight(me.$el.height()) }),
                                me.disableDrag(),
                ...
                (this.toSmallSize = function () {
                    ...
                    var css = me.$el.attr("old-style").getCss();
                    return (
                        me.$el.animate({ left: css.left, top: css.top, width: css.width, height: css.height, right: css.right, bottom: css.bottom }, me.$options.collapseAnimation, function () {
                            ...
                            var bWidth = "",
                                bHeight = "";
                            me.isPinned()
                                ? "auto" !== me.$options.bodyHeight && ((bHeight = me.$options.bodyHeight), _saveState("pinned"))
                                : ((bWidth = _calculateBodyWidth(me.getWidth())), (bHeight = _calculateBodyHeight(me.getHeight())), _saveState("unpinned")),
                ...
                (this.setPosition = function (left, top) {
                    return me.isPinned() ? me : (me.$el.animate({ left: left, top: top }, 100), me);
                }),
                (this.setWidth = function (w) {
                    if (me.isPinned()) return me;
                    var bWidth = _calculateBodyWidth(w);
                    return me.$el.animate({ width: w }, 100), $body.animate({ width: bWidth }, 100), me;
                }),
                (this.setHeight = function (h) {
                    if (me.isPinned()) return me;
                    var bHeight = _calculateBodyHeight(h);
                    return me.$el.animate({ height: h }, 100), $body.animate({ height: bHeight }, 100), me;
                }),
                (this.setSize = function (w, h) {
                    if (me.isPinned()) return me;
                    var bHeight = _calculateBodyHeight(h),
                        bWidth = _calculateBodyWidth(w);
                    return me.$el.animate({ height: h, width: w }, 100), $body.animate({ height: bHeight, width: bWidth }, 100), me;
                }),
                ...
                (this.enableResize = function () {
                    var handles = !1;
                    return (
                        "vertical" === me.$options.resize ? (handles = "n, s") : "horizontal" === me.$options.resize ? (handles = "e, w") : "both" === me.$options.resize && (handles = "all"),
                        handles
                            ? (me.$el.resizable({
                                  ...
                                  resize: function () {
                                      ...
                                      $body.css({ width: bWidth, height: bHeight }), _triggerEvent("onResize");
                                  },
                              }),
                              me)
                            : void 0
                ...
                (this.load = function (params) {
                    (params = params || {}), "string" == typeof params && (params = { url: params });
                ...
                (this.$el = $el),
                me.$el.data("inner-id") || ((me.hasRandomId = !0), me.$el.attr("data-inner-id", Math.randomString(10))),
                (innerId = me.$el.data("inner-id")),
                me.hasRandomId || ((me.storage = localStorage.getItem(storagePrefix + innerId)), (me.storage = JSON.parse(me.storage) || {})),
        ...
        ($.fn.lobiPanel = function (option) {
            ...
            (LobiPanel.PRIVATE_OPTIONS = { parentAttr: "data-lobipanel-child-inner-id", toolbarClass: "lobipanel-minimized-toolbar", initialZIndex: 1e4, iconClass: "panel-control-icon" }),
            ($.fn.lobiPanel.DEFAULTS = {
                ...
                minimize: { icon: "glyphicon glyphicon-minus", icon2: "glyphicon glyphicon-plus", tooltip: "Minimize" },
                expand: { icon: "glyphicon glyphicon-resize-full", icon2: "glyphicon glyphicon-resize-small", tooltip: "Fullscreen" },
                close: { icon: "glyphicon glyphicon-remove", tooltip: "Close" },
                editTitle: { icon: "glyphicon glyphicon-pencil", icon2: "glyphicon glyphicon-floppy-disk", tooltip: "Edit title" },
            }),
            $(".lobipanel").lobiPanel();
    });
