...
class AppointmentExport implements FromCollection,WithHeadings,ShouldQueue {
	...
	public function headings(): array {
        return ['Sr. No.','Appointment Date','Time','Checkout Date','Order ID','Disease','Advise','Lab Test','Diagnostic Imaging','Doctor Name  (Id) (Mobile) Speciality Name','Patient Name  (Pid) (Mobile)','Gender/Age','Type','Doc Fee To Pay','Consultation Fee (Rs.)','Total Pay (Rs.)','Payment Status','From','Rating','Organization','Created At','Created At Time','Status','Payment Status','Appointment Status','Waiting Time Hour:Minut','Ref Code'];
    }
	...
	public function setValues($appointments){
		...
		foreach($appointments as $i => $element) {
			...
			// $refData = @$element->UserPP->UsersSubscriptions->ReferralMaster->code;
			// if(!empty($refData)) {
				// $refCode = $refData->code;
			// }
			// else {
				// $refCode = getSubsRefCodeByUserId($element->pId);
			// }
			$pmbl = $mobileShow ? $element->Patient->mobile_no : '*******'.substr($element->Patient->mobile_no,7);
			$dmbl = $mobileShow ? $user->DoctorInfo->mobile : '*******'.substr($user->DoctorInfo->mobile,7);
			$appointmenArray[] = array(
				 $i+1,
				 date('d-m-Y',strtotime(@$element->start)),
				 date('H:i:s',strtotime(@$element->start)),
				 !empty($element->check_out) ? date('d-m-Y H:i:s',@$element->check_out) : '',
				 @$appointmentOrder->id,
				 getChiefComplaints(@$element->chiefComplaints),
				 getPatientAdvise(@$element->PatientAdvice),
				 $userLabs,
				 $vaR,
				 @$user->DoctorInfo->first_name." ".@$user->DoctorInfo->last_name." (".@$user->id.") (".$dmbl.")".$accNo. ",".@$user->DoctorInfo->docSpeciality->specialities,
				 // @$user->DoctorInfo->getStateName->name,
				 // @$user->DoctorInfo->getCityName->name,
				 @$meta_data->patient_name." (".@$element->pId.") (".$pmbl.")",
				 ...
				 date('H:i:s',strtotime(@$element->created_at)),
				 $status,
				 $pStatus,
				 $wsts,
				 $waitingTime,
				 isset($element->UserPP->UsersSubscriptions) && $element->UserPP->UsersSubscriptions->order_status == 1 ? @$element->UserPP->UsersSubscriptions->ReferralMaster->code : ''
			);
		 }
