...
class SettingsController extends Controller
{
	...
	public function addSubAdmin(Request $request)
	{
		if ($request->isMethod('post')) {
			...
			if ($request->hasFile('logo')) {
				...
				$filename = $docPath . $data['logo_old'];
				...
				// storeFileAwsBucket($docPath, $fileName, file_get_contents($image));
				$request->file('logo')->move(public_path('/doctor'), $fileName);
			} else {
				$fileName = $data['logo_old'];
			}

			if (empty($admin)) {
				$ref = isset($data['code']) ? implode(',', $data['code']) : null;
				Admin::create([
					'name' => $data['name'],
					'email' => $data['email'],
					'mobile_no' => $data['mobile_no'],
					'city' => $data['city'],
					'shift_t_strt' => $data['shift_t_strt'],
					'shift_t_end' => $data['shift_t_end'],
	...
	public function addOrganization(Request $request)
	{
		if ($request->isMethod('post')) {
			...
			$data1 = OrganizationMaster::create([
				'title' => $data['title'],
				'slug' => $data['slug'],
				'logo' => $fileName,
				'pwd' => $pwd,
	...
	public function updateReferralsMaster(Request $request)
	{
		if ($request->isMethod('post')) {
			...
			$ref = ReferralMaster::where('code', $data['code'])
				->where(['status' => 1, 'delete_status' => 1])
				->where('id', '!=', $id)
				->first();

			if (!empty($ref)) {
				return response()->json(['status' => 'error', 'message' => 'Entered code already exists'], 400);
			} else {
				$plan_ids = null;
				if (!empty($data['plan_ids'])) {
					$ids = [];
				...
				try {
					ReferralMaster::where('id', $id)->update([
						'title' => $data['title'],
						'referral_discount_type' => $data['referral_discount_type'],
						'referral_discount' => $data['referral_discount'],
						'code' => $data['code'],
					...
					return response()->json(['status' => 'success', 'message' => 'Referral Code Updated Successfully'], 200);
				} catch (\Exception $e) {
					return response()->json(['status' => 'error', 'message' => 'System Problem: Please try again later.'], 500);
				}
			}
		}
	...
	public function deleteReferralMaster($id)
	{
		...
		ReferralMaster::where('id', base64_decode($id))->update(['delete_status' => '0']);
