...
class CommonController extends APIBaseController {
	...
	public function getAds(Request $request) {
		...
		if($validator->fails()) {
			...
			$dashpop = null;
			// if(getSetting("is_wmh_pop_show")[0] == "1") {
				// $dashpop = DB::table('pages')->where(["status"=>1,'slug'=>'dashboard-pop-up'])->where("lng",$user_array['lng'])->first();
			// }
			$pages = [$page,$page1];
			$walletDetails = UserDetails::select('wallet_amount','referral_code')->where(['user_id'=>$user_array['user_id']])->first();
			$unIds = UserNotifications::select(['id'])->pluck('id')->sort();
			...
			$is_enable_maintenance = getSetting("is_enable_maintenance")[0];
			$mntTxt = '';
			if($is_enable_maintenance){
				if($user_array['lng'] == "hi"){
					$mntTxt = getSetting("maintenance_hi")[0];
				}
				else{
					$mntTxt = getSetting("maintenance_eng")[0];
				}
			}
			$is_mh_enable = getSetting("is_mh_enable")[0];
			$refText = getSetting("refer_text_app")[0];

			$deleteReqMsg = null;
			$deleteTimer = null;
			$is_delete = 0;
			$delReq = DeletedRequest::where('user_id',$user_array['user_id'])->where(['delete_status'=>0,'status'=>1])->first();
			if(!empty($delReq)) {
				$is_delete = 1;
				$req_date = date('Y-m-d H:i:s', strtotime($delReq->created_at));
				// Add 3 days to the request date
				$deletion_date = (new \DateTime($req_date))->modify('+3 days');
				// Get the current date and time
				$current_date = new \DateTime();
				// Calculate the difference
				$diff = $current_date->diff($deletion_date);
				// Check if the deletion date is in the future
				if ($current_date < $deletion_date) {
					$deleteReqMsg =  $diff->format('Your account will be deleted in %a days, %h hours, %i minutes, %s seconds');
				} else {
					$deleteReqMsg = "The account deletion time has already passed.";
				}
				$deleteTimer = $diff->format('%a D, %h H, %i M, %s S');
			}
			$response = ["ads"=>$banners,"banner"=>$pages,"walletDetails"=>$walletDetails,"notificationIcon"=>$readAllN,"wmh_popup"=>$is_wmh_pop_show,"dashboard_popup"=>$dashpop,"is_vdo_req"=>$isVdoReq,"refer_text_app"=>$refText,'is_mh_enable'=>$is_mh_enable,'mntTxt'=>$mntTxt,'deleteReqMsg'=>$deleteReqMsg,'is_delete_req'=>$is_delete,'deleteTimer'=>$deleteTimer];
	...
	public function updateBpRecordDetails(Request $request) {
		if($request->isMethod('post')) {
			...
			$user_array['date'] = $data->get('date');
			$user_array['time'] = $data->get('time');
			$user_array['ques_meta'] = $request->all('ques_meta');
	...
	public function fetchNotification(Request $request) {
		...
		if($validator->fails()){
			...
			$notifications = UserNotifications::select(['title','message','id','route','meta_data','icon','created_at'])->with("NotificationUserId")->paginate(10);
