...
class HomeController extends APIBaseController {
	...
	function searchDoctors(Request $request) {
		...
		if($user_array['search_key'] != '') {
			...
			if($user_array['lng'] == "hi") {
				$symptomps_query = Symptoms::with(['SymptomsSpeciality','SymptomTags'])->Where(['status'=>1])->Where(['mh_status'=>0])->Where('symptom_hindi', 'like', '%'.$search_key.'%');
				...
				$symptomps_query = Symptoms::with(['SymptomsSpeciality','SymptomTags'])->Where(['status'=>1])->Where(['mh_status'=>0])->Where('symptom', 'like', '%'.$search_key.'%');
				$symptomps_query->OrWhereHas("SymptomTags",function($qry) use($search_key) {
						$qry->Where('text','like','%'.$search_key.'%');
				});
				$doc_data_by_symptomps = $symptomps_query->limit(5)->get();
	...
	public function searchDoctorsByFilters(Request $request) {
		...
		$user_array['speciality'] = $request->all('speciality');
		$user_array['group_id'] = $data->get('group_id');
		$user_array['search_key'] = $data->get('search_key');
		$user_array['city'] = $data->get('city');
		$user_array['locality'] = $data->get('locality');
...
public function checkFirstDirectTeleAppointment(Request $request) {
	...
	public function staticPages(Request $request) {
		...
		if($user_array['_from'] == '1') {
			if(!empty($user_array['lng'])) {
				$page = DB::table('pages')->where(["status"=>1,'slug'=>'direct-appointment-app-vaani'])->where("lng",$user_array['lng'])->first();
			}
			if(empty($page)) {
				$page = DB::table('pages')->where(["status"=>1,'slug'=>'direct-appointment-app-vaani'])->where("lng","en")->first();
			}
		}
		else {
			if(!empty($user_array['lng'])) {
    ...
    function getRecordingStatus(Request $request) {
        if($request->isMethod('post')) {
            ...
            if($validator->fails()){
                ...
                curl_close($curl);
				if(!empty($sid)) {
					DB::table('pages')->where("sid",$sid)->update([
	...
	function fetchReviews(Request $request) {
		...
		if($validator->fails()) {
			...
			$feedback = PatientFeedback::where('doc_id',$user_array['doc_id'])->where(["status"=>1,"delete_status"=>1,"publish_admin"=>1])->orderBy('rating')->get()->toArray();
			
			$newArr = [];
			if(count($feedback) > 0) {
				foreach($feedback as $raw) {
					$suggestion_array = ['id' => null, 'name' => null];
					if(!empty($raw['suggestions'])) {
						$suggestions = explode(",",$raw['suggestions']); 
						if(count($suggestions) > 0) {
							foreach($suggestions as $sugs) {
								$suggestion_array[] = array("id"=>$sugs,'name'=>getComplimentName($sugs));	
							}
					...
					$raw["suggestion_array"] = $suggestion_array;
					$newArr[] = $raw;
				}
			}
			$perPage = 5;
			...
			$feedback =  new Paginator($itemsForCurrentPage, count($newArr), $perPage,Paginator::resolveCurrentPage(), array('path' => Paginator::resolveCurrentPath()));

			return $this->sendResponse($feedback, 'Feedbacks fetch successfully.',true);
		}
	}
	...
	public function deleteUserAccount(Request $request) {
        ...
        if(!empty($userData)) {
			...
			$deletedRequest = DeletedRequest::create([
                'reason' => implode(',',$user_array['reason']),
                'user_id' => $user_array['user_id'],
            ]);
			return $this->sendResponse('','Thank you for acknowledging your account deletion request.',true);
        } else {
	...
	public function withdrawalReq(Request $request){
		...
		if($validator->fails()){
			...
			DeletedRequest::where("user_id",$user_array['user_id'])->update(['status'=>0]);
			return $this->sendResponse('','Your account deletion request withdrawal successfully',true);
		}
	}
	/**
	...
	public function videoLanding(Request $request)
{
    try {
		...
		$extOrderId = "health-gennid-vht-" . str_pad(1 + 1, 8, '0', STR_PAD_LEFT); 
		// $extOrderId = "health-gennie-vht-00100109";
        // $extOrderId = "health-gennie-ext-01012025";
        $orgId = "YdE7c5f6";
        // $thirdKey = "health-gennie-01012037";
        $thirdKey = "health-gennid-" . str_pad(1 + 7, 8, '0', STR_PAD_LEFT);
