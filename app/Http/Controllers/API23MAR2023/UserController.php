...
class UserController extends APIBaseController {
	...
	public function login(Request $request) {
		...
		if($validator->fails()) {
			...
			if($data['loginInput'] == "7691079774" || $data['loginInput'] == "8905557252") {
				$otp = "123456";
			}
	...
	public function otpVerified(Request $request) {
		...
		$user_array['isCheckTerm'] = $data->get('isCheckTerm');
			...
			if(!empty($userOTP)) {
				if($user_array['otp'] == $userOTP->otp) {
					if($currentDate <= $userOTP->expiry_date) {
						...
						if(!empty($user_array['isCheckTerm'])) {
							$is_check_terms =  $user_array['isCheckTerm'] == true ? 1 : 0;
							UserDetails::Where(['user_id'=>$user->id])->update(['is_check_terms'=>$is_check_terms]);
						}
	...
	public function saveUserLocation(Request $request) {
		$data=Input::json()->all();
		$user_array=array();
		$user_array['id']=$data['user_id'];
		$user_array['location']=$data['loc_data'];
