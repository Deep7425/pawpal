...
class SubscriptionController extends APIBaseController {
	...
	public function getTharapyPlan(Request $request) {
		$data = Input::json();
		$user_array=array();
		$user_array['lng'] = $data->get('lng');
		$user_array['user_id'] = $data->get('user_id');
		$user_array['is_subs'] = $data->get('is_subs');
		$validator = Validator::make($user_array, [
			// 'lng' => 'required'
		]);
		if($validator->fails()){
			...
			if($user_array['lng'] == "hi") {
				if($user_array['is_subs'] == 1) {
					$plan = Plans::where('id',49)->first();
				}
				else{
					$plan = Plans::where('id',46)->first();
				}
			}
			else{
				if($user_array['is_subs'] == 1) {
					$plan = Plans::where('id',48)->first();
				}
				else{
					$plan = Plans::where('id',45)->first();
				}
			}
			return $this->sendResponse($plan,'',true);
		}
