...
class MentalHealthController extends APIBaseController {
	...
	public function setTitleDescProgramHindi($id,$totWeek,$completeWeekCount) {
		...
		if($id == 440) {
			...
			$titleDesc['title'] = "निर्णय लेने के लिए ".$totWeek." सप्ताह का कार्यक्रम";
			if($completeWeekCount > 0) {
				$titleDesc['desc'] = "बधाई हो, आपने ".$completeWeekCount." सप्ताह का कार्यक्रम पूरा कर लिया। अब, नए सप्ताह की ओर बढ़ें।";
			}
			$titleDesc['icon'] = getPath("public/mh-weekly-icons/week-start/decision-making.png");
	...
	public function fetchSession(Request $request) {
		...
		if($validator->fails()){
			...
			$orders = Appointments::with(['Doctors.docSpeciality','Doctors.DoctorData','Doctors.getCityName','Doctors.getStateName','practiceDetails','AppointmentOrder','NotifyUserSms','PatientFeedback','patient','PatientLabs.labs'])->whereIn('pID',$p_ids)->where('delete_status',1)->where('doc_id','!=',2219)->orderBy('start','desc')->paginate(12);
			$success = false;
			$newArr = [];
			if($orders->count()>0) {
				$success = true;
				foreach($orders as $appt) {
					...
					$vdoReq = NotifyUserSms::where(['patient_id'=>$user_array['pid'],'apptId'=>$appt->id])->where('status','0')->whereDate('created_at',date('Y-m-d'))->orderBy("id","DESC")->first();
					$appt['is_video'] = 0;
					$appt['is_voice'] = 0;
					if(!empty($vdoReq)) {
						if($vdoReq->call_type == 1) {
							$appt['is_video'] = 1;	
						}
						else{
							$appt['is_voice'] = 1;
					...
					foreach($raw->MhWeeklyProgram as $col) {
						if(count($col->AssessmentOverview) > 0) {
							foreach($col->AssessmentOverview as $line) {
								$line['audio_file_url'] = null;
								if($user_array['lng'] == "hi") {
									if(!empty($line->audio_file_hindi)) {
										$line['audio_file_hindi'] = getPath("public/mh-audio-files/".$line->audio_file_hindi);
									}
								}
								else{
									if(!empty($line->audio_file)){
	...
	public function moodHistory(Request $request) {
		...
		if($validator->fails()) {
			...
			$res = MhMood::where(['user_id' => $user_array['user_id']])->orderBy('created_at','DESC')->paginate(10);
	...
	public function pushWeeklyProgram(Request $request) {
		$data=Input::json()->all();
  		$user_array=array();
		$user_array['user_id'] = $data['user_id'];
		$user_array['mhp_mid'] = $data['mhp_mid'];
		$user_array['program_id'] = $data['program_id'];
		$user_array['task_value'] = $data['task_value'];
		$user_array['sheet_title'] = $data['sheet_title'];
	...
	public function insertSheetData(Request $request) {
		$data = Input::json()->all();
  		$user_array=array();
		$user_array['user_id'] = $data['user_id'];
		$user_array['sheet_id'] = $data['sheet_id'];
		$user_array['sheet_value'] = $data['sheet_value'];
		$user_array['id'] = $data['id'];
		$validator = Validator::make($user_array, [
			'user_id' => 'required',
			'sheet_id' => 'required',
	...
	public function getMentalHealthPlan(Request $request) {
		...
		if($validator->fails()){
			...
			foreach($plans as $raw) {
				if($raw->id == 45) {
					$raw->content = '<ul>
						<li><strong>Benefits </strong>- Holistic health care plan for psychological well-being</li>
						<li><strong>15 Appointments</strong> -&nbsp;Full one-year Doctor online consultations and Counseling consultations.</li>
						<li><strong>Weekly Programs</strong> -&nbsp;Detailed notes, worksheets, meditative playlist, journaling safe space and Deep insight about your mental health leading to overall well-being</li>
						<li value="NaN"><strong>Upto 5 Counseling Sessions (45 min) (audio/video call sessions as per your convenience) -</strong>&nbsp;In-depth understanding of issues or concerns &ndash; Detailed Supportive therapy and Psychological therapies</li>
						<li><strong>Psychological Assessments</strong> -&nbsp;Diagnosis and detailed report&nbsp;</li>
						<li><strong>1 Personality Test</strong> -&nbsp;Personality assessment for future understanding of self</li>
						<li><strong>24*7 doctors available</strong> -&nbsp;Anytime and Anywhere access</li>
						<li><strong>Flexible session slots</strong> -&nbsp;Rescheduling available for 24hours</li>
					</ul>';
				}
				else if($raw->id == 46) {
					$raw->content = '<ul>
						<li><strong>लाभ</strong>- मनोवैज्ञानिक स्वास्थ्य के लिए समग्र स्वास्थ्य देखभाल योजना</li>
						<li><strong>15 अपॉइंटमेंट</strong> -&nbsp;पूरे एक साल तक डॉक्टर से ऑनलाइन परामर्श और काउंसलिंग परामर्श।</li>
						<li><strong>साप्ताहिक कार्यक्रम</strong> -&nbsp;विस्तृत नोट्स, वर्कशीट, ध्यान संबंधी प्लेलिस्ट, जर्नलिंग सुरक्षित स्थान और आपके मानसिक स्वास्थ्य के बारे में गहन जानकारी जो समग्र स्वास्थ्य की ओर ले जाती है</li>
						<li><strong>5 काउंसलिंग सत्र (45 मिनट) (आपकी सुविधा के अनुसार ऑडियो/वीडियो कॉल सत्र)</strong> -&nbsp;मुद्दों या चिंताओं के लिए गहन समझ &ndash; विस्तृत सहायक चिकित्सा और साइकोलोजिस्ट उपचार</li>
						<li><strong>3 साइकोलोजिस्ट असेसमेंट</strong> -&nbsp;निदान और विस्तृत रिपोर्ट&nbsp;</li>
						<li><strong>1 व्यक्तित्व परीक्षण</strong> -&nbsp;स्वयं की भविष्य की समझ के लिए व्यक्तित्व असेसमेंट</li>
						<li><strong>24*7 डॉक्टर उपलब्ध</strong> -&nbsp;कभी भी कही भी उपलब्ध</li>
						<li><strong>फ्लेक्सिबल सत्र समय</strong> -&nbsp;24 घंटे के लिए पुनर्निर्धारण उपलब्ध</li>
					</ul>';
				}
			}
	...
	public function fetchArticle(Request $request) {
		if($request->isMethod('post')) {
			...
			if($validator->fails()) {
				...
				$nfs = NewsFeeds::whereRaw("find_in_set('1',news_feeds.type)")->where('is_mh',$user_array['_from'])->whereDate('publish_date', '<=', date("Y-m-d"))->orderBy('show_date', 'DESC')->paginate(10);
				if($nfs->count() > 0){
					foreach($nfs as $raw) {
	...
	public function sympDetails(Request $request) {
		$data = Input::json();
  		$user_array=array();
		$user_array['lng'] = $data->get('lng');
		$user_array['smpId'] = $data->get('smpId');
		$validator = Validator::make($user_array, [
			'smpId'   => 'required'
		]);
		if($validator->fails()) {
			...
			$assess_program = (($user_array['lng'] == "hi") ? 'assess_program_hindi as assess_program' : 'assess_program');
			$symp_details = (($user_array['lng'] == "hi") ? 'symp_details_hindi as symp_details' : 'symp_details');
			$symp = Symptoms::select('id','icon',$symptom,$description,$treatment,$cause,$strategy,$assess_program,$symp_details)->with(['SymptomsSpeciality','SymptomTags'])->where('id',$user_array['smpId'])->where('mh_status',1)->Where(['status'=>1])->first();
			return $this->sendResponse($symp, '',true);
		}
    }
	...
	public function createVhtOrder(Request $request) {
		if($request->isMethod('post')) {
			...
			if($validator->fails()) {
				...
				// $thirdKey = "health-gennie-01012037";
				...
				return $this->sendResponse($url,'Order Created Successfully',true);	
			}
		}
	}
	...
	public function vhtOrders(Request $request) {
		 if($request->isMethod('post')) {
			...
			if($validator->fails()) {
				...
				$vhtOrders = VhtOrder::where("user_id",$user_array['user_id'])->whereNotNull("vh_meta_data")->where("delete_status",0)->orderBy("created_at","DESC")->get();
				if($vhtOrders->count() > 0) {
					foreach($vhtOrders as $raw) {
						$vhtData = json_decode($raw->vh_meta_data,true);
						$vhtData['details'] = preg_replace('/<b>.*?<\/b>/', '', $this->addClassInElements($vhtData['details']), 1);
						$vhtData['suggestion'] = $this->addClassInElements($vhtData['suggestion']);
						$vhtData['score'] = $this->getVHTScore($vhtData['result']);
						$raw['vh_meta_data'] = $vhtData;
					}
				}
				return $this->sendResponse($vhtOrders,'Order Fetch Successfully',true);	
			}
		}
	}
	
	public function currentVhtRecord(Request $request) {
		 if($request->isMethod('post')) {
			$data = Input::json();  
			$user_array=array();
			$user_array['user_id'] = $data->get('user_id');
			...
			if($validator->fails()) {
				...
				$arr = ["is_test_done"=>$its,"vhtOrder"=>$vhtOrder];
				return $this->sendResponse($arr,'Order Fetch Successfully',true);	
			}
		}
	}
	...
	public function updateVhtRecord(Request $request) {
		 if($request->isMethod('post')) {
			$data = Input::json();  
			$user_array=array();
			$user_array['user_id'] = $data->get('user_id');
			$user_array['ext_order_id'] = $data->get('ext_order_id');
			
			$validator = Validator::make($user_array, [
				'user_id'   => 'required',
				'ext_order_id'   => 'required'
			]);
			if($validator->fails()) {
				return $this->sendError($validator->errors());
			}
			else {
				VhtOrder::where('user_id',$user_array['user_id'])->where("ext_order_id",$user_array['ext_order_id'])->update(['is_view'=>1]);
				return $this->sendResponse(1,'Order Update Successfully',true);	
			}
		}
	}
	
	public function fetchVHDashboardData(Request $request) {
		 if($request->isMethod('post')) {
			$data = Input::json();  
			$user_array=array();
			$user_array['user_id'] = $data->get('user_id');
			...
			if($validator->fails()) {
				...
				// No significant abnormality = 0 - 10, Mild = 10 - 20, Moderate = 20 - 30, Moderate To Severe = 30 - 40 and Severe = 40 - 50
				$vhtOrder = VhtOrder::where("user_id",$user_array['user_id'])->whereNotNull("vh_meta_data")->where("delete_status",0)->orderBy("created_at","DESC")->first();
				if(!empty($vhtOrder)) {
					$vhtData = json_decode($vhtOrder->vh_meta_data,true);
					$vhtData['score'] = $this->getVHTScore($vhtData['result']);
				...
				$pnmh = DB::table('pages')->where(["status"=>1,'slug'=>"privacy-notice-ai-based-mantel"])->where("lng","en")->first();
				
				$vhtOrders = VhtOrder::where("user_id",$user_array['user_id'])->whereNotNull("vh_meta_data")->where("delete_status",0)->orderBy("created_at","DESC")->get();
				$scoringArr = [];
				if($vhtOrders->count() > 0) {
					$scoringArr[] = ["score"=>0,"result"=>"No significant abnormality","date"=>$vhtOrders[0]['created_at']];
					foreach($vhtOrders as $raw) {
						$vhtData = json_decode($raw->vh_meta_data,true);
						$score = $this->getVHTScore($vhtData['result']);
						$scoringArr[] = ["score"=>$score,"result"=>$vhtData['result'],"date"=>$raw->created_at];
					}
				}
				$settingData = Settings::whereIn('key',["is_video_app_show","vdo_name"])->pluck("value","key");
				$isVdoAppShow = $settingData["is_video_app_show"];
				$vdo_url = null;
				if($isVdoAppShow == 1) {
					$vdo_url = getPath("public/uploads/ads_video/".$settingData["vdo_name"]);
				}
				$arr = ["last_order"=>$vhtOrder,"policy"=>$pnmh,"past_history"=>$scoringArr,"ads_video"=>$vdo_url];
				return $this->sendResponse($arr,'Order Fetch Successfully',true);	
			}
		}
	}
