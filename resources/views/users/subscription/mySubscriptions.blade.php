          ...
          <thead>
            <tr>
              <th class="plan-name">Plan Name</th>
				<th class="transaction-number">Transaction Number</th>
				<th class="payment-mode">Payment Mode</th>
				<th class="tax">Tax</th>
				<th class="payble-amount">Payble Amount</th>
          ...
          <tbody>
            @foreach($UsersSubscriptions as $index => $subs)
              ...
              <td>@if($subs->order_status == "0") pending @elseif($subs->order_status == "1") completed @elseif($subs->order_status == "2") Cancelled @elseif($subs->order_status == "3") Failure Transaction @endif</td>
			   <td>@if(@$subs->PlanPeriods->status == "1") Active @else Deactivated @endif</td>
              <td>
