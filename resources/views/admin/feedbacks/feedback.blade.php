                ...
                <div class="row ml-1 form-top-row">
                    ...
                    <div class="btn-group excel">
                        ...
                        <div class="ml-sm-2">
                            ...
                            {!! Form::open(array('route' => 'admin.feedbackPatAll', 'id' => 'chnagePagination',
                            'method'=>'POST')) !!}
                        ...
                        <table class="table table-bordered table-hover">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td>Dr. {{@$element->Doctors->first_name}} {{@$element->Doctors->last_name}}</td>
                                    <td>@if(!empty($element->user_id)){{getUserName($element->user_id)}}@endif</td>
                                    <td>{{@$element->User->mobile_no}}</td>
                                    <td>@if($element->visit_type == '1') Consultation @elseif($element->visit_type ==
                                        '2') Procedure @elseif($element->visit_type == '3') Follow up @endif</td>
                                    <td>
                                    ...
                                    <td>
                                        @php $suggestions = explode(",", $element->suggestions); @endphp
                                        @if(!empty($element->suggestions))
                                        @foreach ($suggestions as $value)
                                       {{getCompliments($value)}}
                                        @endforeach
                                        @else
                                        Not Available
                                        @endif
                                    ...
                                    <td><a class="btn @if($element->publish_admin == '0') btn-success @else btn-danger @endif changeStatus"
                                            type="1" status="{{$element->publish_admin}}" data-id="{{$element->id}}"
                                            href="javascript:void();">@if($element->publish_admin == '0') Publish @else
                                            Unpublish @endif</a></td>
                                    <td><a class="btn @if($element->status == '0') btn-success @else btn-danger @endif changeStatus"
                                            type="2" status="{{$element->status}}" data-id="{{$element->id}}"
                                            href="javascript:void();">@if($element->status == '0') Active @else Inactive
                                            @endif</a></td>
                                    ...
                                    <td>
                                        <div style="width: 75px;">
                                            ...
                                            <a style="margin-right:5px; float:left;" href="javascript:void();" class="btn btn-info btn-sm"
                                                onclick="viewFeedback({{$element->id}});"><i style="margin-top:3px;" class="fa fa-eye"
                                                    aria-hidden="true"></i></a>

                                            <a href="javascript:void(0);" pkey="{{base64_encode(@$element->id)}}"r_from="3" class="btn btn-info btn-sm manageSprt"
                                                title="Manage Leads"><img src='{{ url("/img/customer-care-icon.png") }}' /></a>
                                        </div>
                                    </td>
                                </tr>
                                @endforeach
        ...
        if (confirm('Are You sure want to confirm this appointment.')) {
            ...
            jQuery.ajax({
                ...
                success: function(result) {
                    ...
                    if (result == 1) {
                        jQuery('.loading-all').hide();
                        $(confirm_btn).closest('td .confirm_by_doc').addClass(
                            'label-success');
                        $(confirm_btn).closest('td .confirm_by_doc').removeClass(
                            'label-warning');
