...
<html lang="en">
 ...
 .form-fields.typeField {
    ...
    margin: 8px auto;
    display: table;
    border: 1px solid #3766b8;
}
.form-fields.typeField label {
    width: 100%;
    padding: 0px 0px 7px 0px;
    margin: 0px;
    text-align: left;
}
.form-fields.typeField label i { color: red;}
.form-fields.typeField .radio-wrap { width: 50%; float:left; padding: 0px 0px 0px 0px;text-align: left;}
.form-fields.typeField .radio-wrap label { width: auto; padding: 0px;}
.form-fields.typeField .radio-wrap input { float:left; margin-right: 5px;}
    </style>
 </head>
 <body>
        ...
        <div class="tabs-reload tab-pane active">
          <div class="right-box">
             <div class="aad-inventory-section">
               ...
               <h3 style="color: #3766b8; margin-bottom: 5px; float: none;  width: 100%;  border-bottom: 0px solid #3766b8;  padding: 0 0 8px;  font-weight: 600; text-align: center;  font-size: 30px; margin: 0 auto !important;">Book Appointment Online</h3>

    <h4 style="color: #222; margin-bottom: 50px; float: left; width: 100%; padding: 0 0 8px; font-weight: 600; text-align: center;
    font-size: 16px;
">Through Health Gennie QR Code</h4>
               ...
               <p style="width:auto; border: 2px solid #3766b8; border-radius: 3px; display:inline-block;">
                <img style=" max-width:100%;"  src="data:image/png;base64, {!! base64_encode(QrCode::format('png')->merge(URL::asset('img/qrlogoGennie.png'), 0.3, true)
               ...
               <div class="top-section-search">
			    <button type="button" class="btn btn-default printqrCode" style="border-radius: 3px !important;
				 ...
				 <div class="form-fields typeField">
					<label>Print Format <i class="required_star">*</i></label>
					<div class="radio-wrap">
					<input type="radio" class="qrCodeType" name="qrCodeType" value="1" checked />
					<label>English</label>
					</div>
					<div class="radio-wrap">
					<input type="radio" class="qrCodeType" name="qrCodeType" value="2" />
					<label>Hindi</label>
					</div>
				  </div>
             </div>
...
<form method="post" action="{{route('qrcode.index',['action'=>'doctor', 'id'=>$user->DoctorSlug->name_slug])}}" accept-charset="UTF-8" id="qrCodeIndex">
	 @csrf
...
<script>
  $(document).on("click", ".printqrCode", function (e) {
	var qRtype = $('input[name=qrCodeType]:checked').val();
  	var action=  "{{Request::route('action')}}";
    jQuery.ajax({
    type: "POST",
    data:{id:'{{$user->id}}',qRtype:qRtype,'action':action},
    ...
    success: function(res) {
      ...
      $('.loading-all').hide();
      $('.printData').html(res);
      var nn = $('.printData').html();
      //$('.printData').printMe({ "path": ["https://www.healthgennie.com/css/qrPrint.css"] });
...
setTimeout(function() {
    ...
    newWindow.close();
  	...
  	$(document).on("click", ".downloadCode", function (e) {
		$('.loading-all').show();
		jQuery("#file_type").val("pdf");
		jQuery("#qrCodeIndex").submit();
