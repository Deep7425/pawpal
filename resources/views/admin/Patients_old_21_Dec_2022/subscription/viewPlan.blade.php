		...
		<div class="modal-body">
			<div class="panel panel-bd lobidrag">
				<div class="panel-heading">
						<a class="btn btn-primary" href="javascript:void(0);">Subscription No. : <strong>{{$UsersSubscriptions->id}}</strong> </a>
						<a class="btn btn-primary status" href="javascript:void(0);">Order Status : <strong>@if($UsersSubscriptions->order_status == "0") pending @elseif($UsersSubscriptions->order_status == "1") completed @elseif($UsersSubscriptions->order_status == "2") Cancelled @elseif($UsersSubscriptions->order_status == "3") Failure Transaction @endif</strong></a>
						<a class="btn btn-primary status" href="javascript:void(0);">Subscription Date : <strong>
						{{date('F d Y g:i A',strtotime($UsersSubscriptions->created_at))}}</strong></a>
				</div>
				  ...
				  @if($UsersSubscriptions->UserSubscribedPlans->count() > 0)
					@foreach($UsersSubscriptions->UserSubscribedPlans as $index => $plan)
					  <tr>
						...
						<td>@isset($plan->PlanPeriods->start_trail){{$plan->PlanPeriods->start_trail}}@endisset</td>
        ...
        <div class="panel-heading PriceDiv">
          ...
          <tbody>
          	<tr>
          		...
          		<td style="text-transform: uppercase;"><strong> {{$UsersSubscriptions->subcribedate}}</strong></td>
          	</tr>
          	<tr>
