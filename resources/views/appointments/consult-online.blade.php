...
<div class="container wrapper-appoint">
	...
	<?php $user = Auth::user(); $is_subscribed = 0; $conType = 1; ?>
	@if(isset($doctor) && !empty($doctor))
		<?php
		  ...
		  // else {
			...
			$convFee = 0;
			$total_fee = $consultation_fees;
		  // }
		  $docFee = 50;
		?>
		 <input class="doc_id_app_book" type="hidden" value="{{base64_encode($doctor->id)}}"/>
		 <div class="doctor-listtop doctor-listtop2 ap-section-new RegistrationFeeTop">
			{!!getTermsBySLug('online-consultation','en')!!}
			<div class="consult-fee-div"><span>Registration Fee : </span>
				<!--<p>₹ <del>{{$consultation_fees}}</del> /-  FREE</p>-->
				<p>FREE</p>
			</div>
		 </div>
		 @if($doctor->claim_status == '1' && $doctor->varify_status == '1')
		 <div class="from-widget-top from-widget-topnew" style="@if($app_id != '') display:none; @else  display:block @endif">
			{!! Form::open(array('name' => 'appointmentForm','url' => route('bookConsult'), 'method'=>"POST" )) !!}
        <ul class="serverError" style="color:red;"></ul>
				<div class="appointment-popup-block22">
					 ...
					 <input type="hidden" name="doctor" value='{{base64_encode($doctor->id)}}'>
					 <input type="hidden" name="date" value="{{date('d-m-Y')}}">
					 <input type="hidden" name="time" value='{{strtotime(date("h:i A"))}}'>
				 ...
				 <h2 class="This-Appointment">This Appointment is for:<i class="required_star">*</i></h2>
						<div class="form-fields  form-field-mid pad-r0 specialization">
						 ...
						 <div class="ThisAppointment">
				              <h2 class="This-Appointment">Please Fill Patient Information:</h2>
						...
						<!--<div class="from-widget-section dob-wrapper">
						 ...
						 <div class="from-widget-section">
							 <label>Organization<i class="required_star">*</i></label>
							 <input class="" type="text" name="organization" placeholder="Organization" value=""/>
							 <span class="help-block"></span>
						 </div>
						 </div>
				</div>
				 ...
				 @endif
				  <input type="hidden" name="is_subscribed" value='{{$is_subscribed}}'/>
						...
						<div class="appointment-checkout-table">
							@if($isDirect == '1')
								<table>
									...
									<tr><td>Convenience Fee : </td><td> ₹ {{$convFee}} /-</td></tr>
									<!--<tr><td>Coupon : </td><td> ₹ <span class="coupon_discount">0.00</span> /-</td></tr>-->
									<tr><td>Total Pay : </td>
										<!--<td> ₹ @if($isFree == '0')<span class="total_fee">{{$total_fee}}</span>/- @else <strike class="total_fee">{{$total_fee}}</strike>/- FREE @endif</td>-->
										<td><span class="total_fee">FREE</span></td>
									</tr>
								</table>
							@else
								<table>
									@if($doctor->fees_show == '1' && $conType == "2")
										<tr><td>Registration Fee @if($conType == "2")<strong>(Pay at clinic)</strong>@endif : </td>
									...
									<tr><td>Coupon : </td><td> ₹ <span class="coupon_discount">0.00</span> /-</td></tr>
									<tr><td>Total Pay : </td>
									<td> ₹ <span class="total_fee">{{number_format($total_fee,2)}}</span>/-</td></tr>
								</table>
							@endif			
						</div>
					...
					<div class="from-widget-btn">
						...
						<button type="submit" class="btn btn-default subbtn" next="1" isFree="{{$isFree}}" isDirect="{{$isDirect}}">Continue</button>
					 </div>
			{!! Form::close() !!}
			 </div>
  ...
  data: {'couponcode':couponcode,'consultation_fees':consultation_fees,'onCallStatus':onCallStatus},
  success: function(data){
		if (data.coupon_code) {
