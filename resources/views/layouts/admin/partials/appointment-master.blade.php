@extends('layouts.admin.Masters.Master') @section('title', 'Appointments Master') @section('content')
...
<body>
    ...
    <div class="layout-wrapper layout-2">
         <?php $userModule = checkAdminUserModulePermission(50); ?> <div class="layout-inner">
            <div class="layout-container">
			 <div class="col-sm-12"> <?php $subscribedDocs = getSetting("specialist_doctor_user_ids");?> 
                <div class="layout-content">
                    <!-- [ content ] Start -->
                    <div class="container-fluid flex-grow-1 container-p-y">
                        <h4 class="font-weight-bold  mb-0">Appointment</h4>
                        ...
                        <div class="layout-content">
							...
							<!-- header start -->
								...
								<div class="row" <?php $subscribedDocs = getSetting("specialist_doctor_user_ids");?> >
											...
											<div class="col-sm-1">
												<div class="btn-group">
													<a
														...
														onClick="ForExcel()"
														title="Excel"
														><img src='{{ url("/img/excel-icon.png") }}'
													/></a>
												</div>
											</div>
										</div>
									</div>
									<div class="col-sm-7 ml-5 mt-2 d-flex justify-content-end">
										<div class="btnbtn-success123">
											@if(!isset($_GET['today_appt']))
											<div class="btn-group">
												<a
													class="btn btn-success"
													href="{{ route('admin.hgAppointments',['today_appt'=>base64_encode(1),'start_date'=>base64_encode(date('Y-m-d')),'end_date'=>base64_encode(date('Y-m-d'))]) }}"
													>Past Appointments ({{getTotalAppointmentByDate()}})</a
												>
											</div>
											@else
											<div class="btn-group">
                                    ...
                                    <!-- <label>Page</label>--> {!! Form::open(array('route' => 'admin.hgAppointments', 'id' => 'chnagePagination', 'method'=>'POST')) !!} <input type="hidden" name="today_appt" value="@if(isset($_GET['today_appt'])) {{base64_decode($_GET['today_appt'])}} @endif" />
                                ...
                                <div class="card-body demo-vertical-spacing">
                                    ...
                                    <div class="row">
                                        ...
                                        <select class="form-control" name="user_id" id="exampleSelectMultiple">
                                            <option value="">select Doctor</option> @if(count($practices) > 0) @foreach ($practices as $pra) <option value="{{$pra->user_id}}" @if(old('user_id')==$pra->user_id) selected @endif >{{@$pra->first_name}} {{@$pra->last_name}} ({{$pra->email}}) @if(in_array($pra->user_id,$subscribedDocs))(PLAN)@endif</option> @endforeach @endif
                                        </select>
                                    </div>
                                        </div>
                                        <!-- Payment From -->
                                        ...
                                        <select class="form-control" name="code">
                                            <option value="">Select</option> @if(count(getOrganizations())>0) @foreach(getOrganizations() as $raw) <option value="{{$raw->id}}" @if(isset($_GET['code'])) @if(base64_decode($_GET['code'])==$raw->id) selected @endif @endif>{{$raw->title}}</option> @endforeach @endif
                                        </select>
                                      </div>
                                        </div>
                                        <!-- Organisation end-->
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-12">
                                <div class="card">
                                    ...
                                    <div class="card-body">
                                        ...
                                        <div class="table-responsive">
                                            <table id="report-table" class="table table-bordered table-striped mb-0">
                                                ...
                                                </thead> @if($appointments->count() > 0) @foreach($appointments as $index => $element) <?php $working_status =  json_decode($element->working_status);
									...
									?> @if(!empty($element->AppointmentOrder) && !empty($element->AppointmentOrder->meta_data)) @php $meta_data = json_decode($element->AppointmentOrder->meta_data); $ldocN = "Dr. ".$meta_data->doc_name; @endphp @endif <tr class="tr-appointment" @if($is_next=='1' ) style="background-color: #e89e44;" @endif @if(in_array($element->Patient->mobile_no,['9739977204','8826897971','9636257186','9769148411','9414430699'])) style="background-color: #7fff3133;" @endif > <td>
                                                        <label>{{$index+($appointments->currentpage()-1)*$appointments->perpage()+1}}.</label>
                                                    </td>
                                                    <td>{{date('d-m-Y h:i A',strtotime($element->start))}} @if(!empty($element->NotifyUserSms) && $element->NotifyUserSms->status == '0')</br>
                                                        <div class="fa-video-camera123 blink">
                                                            <i class="fa fa-video-camera" aria-hidden="true" title="video calling.."></i>
                                                        </div> @elseif(!empty($element->NotifyUserSms) && $element->NotifyUserSms->status == '1')</br>
                                                        <div class="fa-video-camera123">
                                                            <i class="fa fa-video-camera" aria-hidden="true" title="video calling.."></i>
                                                        </div> @endif <br>{{@$element->AppointmentOrder->id}}
                                                    </td>
                                                    <td>{{getChiefComplaints($element->chiefComplaints)}}</td>
                                                    <td> @if(@$element->PatientLabsOne->appointment_id) <strong style="text-align:left; float:left;">Yes </strong> @else No @endif </td>
                                                    <td> @if(@$element->PatientDiagnosticImagings->appointment_id) <strong style="text-align:left; float:left;">Yes </strong> @else No @endif </td>
											...
											?> {{$docFeeToPay}}
                                                    ...
                                                    <td>@if((!empty($element->AppointmentTxn) && @$element->AppointmentOrder->type == "1") || empty($element->AppointmentOrder)) Paid @elseif(@$element->AppointmentOrder->type == "0") Free @elseif(@$element->AppointmentOrder->type == "2") Cash @endif @if(checkAppointmentIsElite($element->id,@$element->AppointmentOrder->order_by) == 1) @if(in_array($element->id,$appointmentIds)) )<span style="color: #ef6666; font-weight: bold;">(PLAN)</span> @endif <p title="Remaining Specialist Appointment">{{$specialist_appointment_cnt}}</p> @endif @if($element->visit_type == '6') <span style="color: #ef6666; font-weight: bold;">(Follow Up)</span> @endif </td>
                                                    <td> @if(isset($element->AppointmentOrder)) @if($element->AppointmentOrder->order_from == '1') APP @elseif($element->AppointmentOrder->order_from == '0') WEB @elseif($element->AppointmentOrder->order_from == '2') Admin @endif @if(isset($meta_data->isPaytmTab) && $meta_data->isPaytmTab == "true") (Paytm) @elseif(isset($meta_data->organization)) {{$meta_data->organization}} @endif @else @if($element->app_click_status == '5') APP @elseif($element->app_click_status == '6') WEB @endif @endif @if(isset($element->AppointmentOrder) && $element->AppointmentOrder->hg_miniApp == 1) (Help India) @elseif(isset($element->AppointmentOrder) && $element->AppointmentOrder->hg_miniApp == 2) (DOIT Emitra) @endif </td>
                                                    <td>{{$element->note}}</td>
                                                    <td>{{$element->is_document_uploaded == 1 ? 'Done' : 'Pending'}}</td>
                                                    <td class="ratingDiv">
                                                        <div class="ratingDivTop"> @if(isset($element->AppointmentOrder)) @if(empty($element->AppointmentOrder->rating)) <select class="update_rating" appId="{{base64_encode($element->id)}}">
                                                                <option value="">Select</option>
                                                            ...
                                                            <p class="text-success starDiv" style="display:none;">{{$element->AppointmentOrder->rating}} STAR</p> @else <p class="text-success starDiv">{{$element->AppointmentOrder->rating}} STAR</p> @endif @endif @if(!$working_status) <select class="update_appoint" apptId="{{$element->id}}">
                                                            ...
                                                            <span class="cancel-btn label-default label label-success show_working_status" style="display:none;">{{"Closed"}}</span> @else <span class="cancel-btn label-default label label-success show_working_status">{{"Closed"}}</span> @endif
                                                        </div>
                                                        <br>
                                                    </td>
                                                    ...
                                                    <td>
                                                        <span class="cancel-btn label-default label @if($element->status != '1') label-danger @elseif($element->appointment_confirmation == '1') label-success @else label-warning @endif">@if($element->status != '1') Cancelled @elseif($element->appointment_confirmation == '1') Confirmed @else Pending @endif</span> @if($element->status == '0') @if($element->cancel_reason == "other") <span class="text-success">{{$element->other_cancel_reason}}</span> @else <span class="text-success">{{$element->cancel_reason}}</span> @endif @endif @if(!empty($working_status) && $working_status->status == 1) <span class="label-default label label-success">Working...</span> @endif @if($element->visit_status == '1') <span class="text-success">Done</span> @else <span style="color: red">Pending</span> @endif
                                                    </td>
                                                    <td>
                                                        <div class="viewSubscription123 pptHere">
                                                            <ul>
																...
																$slot_duration = round(abs($to_time - $from_time) / 60,2);
																 ?>
																  <li><span slot_duration="{{$slot_duration}}" app_id="{{base64_encode($element->id)}}" pId="{{$element->pId}}" user_id="{{$user_id}}" start_time="{{date('Y-m-d',strtotime($element->start))}}" end_time="{{date('Y-m-d',strtotime($element->end))}}" s_time="{{date('H:i',strtotime($element->start))}}" e_time="{{date('H:i',strtotime($element->end))}}" doc_id="{{$element->doc_id}}" app_type="@if($element->type == 3){{base64_encode(1)}}@else{{base64_encode(2)}}@endif" visit_type="{{$element->visit_type}}" class="label-default label label-info switchOrder"><i class="fa fa-exchange" aria-hidden="true"></i> Switch</span></li> @if(isset($element->AppointmentOrder) && $element->AppointmentOrder->switch_apt == '1') <li class="" title="Switched From {{@$ldocN}}"><span>Switched From<br />{{@$ldocN}}</span></li> @endif <li>
																	 <span class="label-default label label-info showApptDtls" data-pid="{{base64_encode($element->Patient->id)}}" data-mobile="{{base64_encode($element->Patient->mobile_no)}}"><i class="fa fa-calendar-check-o" aria-hidden="true"></i>Show All Appointment</span></li> @if($element->type != '3') <li><span app_id="{{base64_encode($element->id)}}" user_id="{{$user_id}}" working-user-id="@if(!empty($working_status)){{$working_status->user_id}}@endif" class="label-default label label-info  confirm_by_doc"><i class="fa fa-check-square-o" aria-hidden="true"></i> Confirm</span></li> @endif <li><span app_id="{{base64_encode($element->id)}}" user_id="{{$user_id}}" working-user-id="@if(!empty($working_status)){{$working_status->user_id}}@endif" class="label-default label label-info cancel_by_team "><i class="fa fa-ban" aria-hidden="true"></i> Cancel</span></li>
                                                                        ...
                                                                        <li><button class="SMsBtn" onclick="smsView({{$element}},'{{date('d-m-Y',strtotime($element->start))}}','{{date('h:i A',strtotime($element->start))}}')"><i class="fa fa-commenting-o" aria-hidden="true"></i>SMS</button></li> @if($element->visit_status == '1') <li><button class="btn btn-info sendPres" appId="{{$element->id}}" title="Share Prescription via whatsApp"><i class="fa fa-share-alt" aria-hidden="true"></i> Share Prescription </button></li> @endif @if($element->visit_status == '1') <li><button class="btn btn-info sendPresToPharmacy" appId="{{$element->id}}" title="Share Prescription To Pharmacy"><i class="fa fa-share-alt" aria-hidden="true"></i>Pres Share To Pharmacy</button></li> @endif @if($userModule && $element->visit_status == '1') <li>
        ...
        <div class="modal-content ">
            ...
            <div class="modal-body">
                <div class="panel panel-bd lobidrag">
                    ...
                    <div class="panel-body SwitchAppointment"> {!! Form::open(array('id' => 'switchAppointment','name'=>'switchAppointment')) !!} <input name="app_type" value="" type="hidden" />
                        ...
                        <div class="form-groupTopMenu">
                            <div class="form-group">
                                ...
                                <select class="form-control chooseDoc" name="doc_id">
                                    <option value="">Select Doctor</option> @if(count($practices) > 0) @foreach($practices as $pra) @if(!empty($pra->oncall_fee) || !empty($pra->consultation_fees)) <option value="{{$pra->user_id}}">{{@$pra->first_name}} {{@$pra->last_name}} ({{@$pra->email}}) @if(!empty($pra->consultation_fees))({{@$pra->consultation_fees}})@endif @if(!empty($pra->oncall_fee))({{@$pra->oncall_fee}}) @endif @if(in_array($pra->user_id,$subscribedDocs))(PLAN)@endif</option> @endif @endforeach @endif
                                </select>
...
<div class="modal fade" id="AddModal" role="dialog" data-backdrop="static" data-keyboard="false">
    <div class="modal-dialog">
        ...
        <div class="modal-content ">
            ...
            <div class="modal-body">
				 {!! Form::open(array('id' => 'addNote','name'=>'addNote')) !!} <input type="hidden" name="note_type" value="2">
 ...
 <script type="text/javascript">
        ...
        jQuery(document).on("click", ".RefundOrder", function(e) {
            $.confirm({
                title: 'Refund!',
                draggable: false,
                content: '' + '<form action="" class="formName">' + '<div class="form-group">' + '<label>Enter Refund Reason</label>' + '<textarea placeholder="Write Refund Reason..." class="refund_reason form-control" maxlength="200" name="refund_reason" required> </textarea>' + '<span  class="FieldError" style="display:none; color:red;"></span>' + '</div>' + '</form>',
                buttons: {
                    formSubmit: {
                        text: 'Submit',
                        btnClass: 'btn-blue',
