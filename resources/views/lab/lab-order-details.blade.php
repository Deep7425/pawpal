                ...
                <table cellpadding="0" cellspacing="0">
                    ...
                    @foreach($order->LabOrderedItems as $product)
                    	<tr>
							...
							@else
								@php $item_type = $product->package_id > 0 ? 'PACKAGE' : 'CUSTOM'; @endphp
								<p class="product-name"> <a href="{{route('LabDetails', ['id'=> base64_encode($product->product_name), 'type' => base64_encode($item_type)])}} ">{{$product->product_name}}</a> </p>
							@endif
                            </td>
                            <td><p>Ideal for individuals aged 41-60 years</p>@if(isset($data['items'][$i]['test_count']))<p class="tets">Tests Included : {{@$data['items'][$i]['test_count']}}</p>@endif</td>
                            @if($coupon_code == "HGSUBSCRIBED")
								<td> ₹ 0</td>
							@else
								<td> ₹ {{$product->cost}}</td>
...
<p class="info-row">
                              ...
                              @if($data['Gender'] == 'M')  Male @else Female @endif<br /></p>
<p class="info-row">
...
@if(!empty($order->appt_date))<p class="info-row">
                            <i class="fa fa-calendar" aria-hidden="true"></i>
                              {{date('F d Y',$order->appt_date)}}<br /></p>@endif
 @if(!empty($order->appt_date))<p class="info-row">
                              <i class="fa fa-clock-o" aria-hidden="true"></i>
                              {{date('h:i A',$order->appt_date)}} : {{date('h:i A',$order->appt_date+3600 )}}</p>@endif
                        </td>
                    </tr>
                </table>
        ...
        <div class="right-block">
            <ul>
                <li>Total
                    <div class="price-tag-wrap new_nir"><span>₹ </span><span class="totalAmount">@if($coupon_code == "HGSUBSCRIBED") 0 @else {{$order->total_amt + $order->discount_amt}}@endif</span></div>
                </li>
				...
				@endif	
				<li>Service Charge
					<div class="price-tag-wrap new_nir"><span>₹</span><span class="coupanDiscountAmount">{{@$data['service_charge']}}</span></div>
				</li>
