...
<div class="lab-test">
  ...
  <div class="container-fluid all-offers">
  	<div class="container">
			<?php
            $offer_data = getThyrocareData("OFFER");
            ?>
      @if($type == "2")
        @if(count($offer_data) > 0 )<h2>Deals of the Day</h2> @endif
      <div class="checkup-package allOffers">
        <?php $offers = 0; ?>
        @if(count($offer_data) > 0 )
        @foreach($offer_data as $offer)
        <div class="checkup-detail item">
          <a href="{{route('LabDetails', ['id'=> base64_encode($offer->common_name), 'type' => base64_encode($offer->type)])}}">
          ...
          <div class="certification">NABL, ISO</div>
            ...
            <div class="test-included">
                <a href="{{route('LabDetails', ['id'=> base64_encode($offer->common_name), 'type' => base64_encode($offer->type)])}}">({{$offer->testCount}}) Tests Included</a>
            </div>
  ...
  <div class="allProfiles">
    ...
    <div class="package-box item">
      <img src="{{ URL::asset('img/thayrocare-logo-dashboard.jpg') }}" alt="banner"  />
      <h3>{{$offer->common_name}}</h3>
        <span class="total_count">Tests Included</span> <b>({{$offer->testCount}})</b>
   ...
   @elseif($type != "2")
    <h2>Health Checkups and Screenings For</h2>
    <div class="screening allGroups">
	  @forelse(getLabPackageByCompany($type) as $raw)
        <div class="checkup-package">
		<div class="checkup-detail">
		  <div class="Lab_Details_Image">
		  <img src="{{ URL::asset('public/others/company_logos') }}/{{$raw->LabCompany->icon}}" alt="banner"/>
		  </div>
	   <div class="blog-wrapper-content">
		<h3>{{strtoupper($raw->title)}}</h3>
		<div class="test-included">
		  ...
		  <div class="book-now"><input type="hidden" class="selectPackage" name="" lab_type="1" lab_company_type="1" value="{{json_encode($raw)}}">
		  <a href="javascript:void(0)"  url="{{route('LabDetails', ['id'=> base64_encode($raw->DefaultLabs->title), 'type' => base64_encode(@$raw->type)]) }}" class="add_to_cart @if($isCart) removeCart @endif" data-name="{{$raw->DefaultLabs->title}}"  Pcode="{{$raw->DefaultLabs->id}}" Pname="{{$raw->DefaultLabs->title}}" data-type="@if($isCart) 2 @else 1 @endif">@if($isCart) Remove @else Book Now @endif</a></div>
		</div>
		</div>
		</div>
	  @empty
		...
		if(response == 1) {
			jQuery('.loading-all').hide();
			 CartAnimate(this);
			  ...
			  var Div = '<div class="list" data-name="'+pkg.name+'"><img src="{{asset("img/OurStore-icon.png")}}"><h5><a href="'+url+'">'+pkg.name+'</a></h5> <span><strong>1 x ₹'+price+'</strong></span><a class="close deleteFromMiniCart" href="javascript:void(0);"  Pcode="'+pkg.code+'" Pname="'+pkg.name+'"><i class="fa fa-times-circle" aria-hidden="true"></i></a> </div>';
