...
<div class="topHeadingSecion">
	...
	@if(Session::get('info_type') == 'doctorsIn')
	<h1>Best Doctors In {{$cName}}
  ...
  <div class="container-inner">
    ...
    <div class="left-content desktop filteredDivDesktop">
      ...
      <div class="block-filter">
        ...
        <div class="sidebar-block-section" id="sidebar-block">
      ...
      <div class="block-filter">
      	...
      	<div class="filter-wrapConsultation">
			 ...
			 @foreach(getTopLocality(Session::get("city_id")) as $locality)
				<li><a href="{{route('findDoctorLocalityByType',[$cityName,$slug,$locality->slug])}}">{{$locality->name}}</a></li>
              @endforeach
			</ul>
      	</div>
      	</div>
    ...
    <div class="right-content @if(count(getSuggestedDoctors())>0) suggested-width @endif">
        ...
        @if(count($sponsoredDoc->DoctorRatingReviews)>0)
          ...
          <div class="cal-doctor"> <a class="btn call_now" href="javascript:void(0);"><img loading="lazy" width="12" height="12" src="{{ URL::asset('img/mobile-icon.png')}}" alt="icon"  />Call Now</a> </div>
        </div>
		...
		@if(count($infoData) > 0)
			...
			<div class="doc-img"><img loading="lazy" width="56" height="56" @if(@$doc->profile_pic != null) src="{{@$doc->profile_pic}}" @else src="{{ URL::asset('img/doc-img.png')}}"@endif alt="Dr. {{ucfirst(@$doc->first_name)}} {{@$doc->last_name}}"  />
			</div>
			</a>
			<div class="right-con-wrapper">
			<div class="profile-detail"> <a data_id="{{$doc->id}}" info_name="{{@$doc->DoctorSlug->name_slug}}"  href="{{route('findDoctorLocalityByType',[$cityName,$stype,@$doc->DoctorSlug->name_slug])}}">
			  ...
			  <ul class="dgree-top">
				...
				<li><img loading="lazy" width="13" src="<?php echo url("/")."/public/speciality-icon/".$doc->docSpeciality->speciality_icon; ?>" alt="icon"  />{{@$doc->docSpeciality->spaciality}} </li>
				@endif
				@if(!empty($doc->qualification))
				<li class="Dgree-section"><p><strong><img loading="lazy" width="13" src="{{ URL::asset('img/degree-ico.png')}}" alt="icon"  /></strong><span>{{@$doc->qualification}}</span></p></li>
				@endif
                <h4><img loading="lazy" width="16" src="{{ URL::asset('img/exp.png')}}" alt="icon"  />Experience : <span>@if(!empty($doc->experience)){{@$doc->experience}} Years @else Not Updated @endif</span></h4>
			  </ul>
            ...
            <div class="doc_available">
				...
				<p class="now"><img loading="lazy" width="15" src="{{ URL::asset('img/availablity-icon.png')}}" alt="icon"  />Available Today</p>
				@endif
			  </div>
              @if(in_array(2,$consult_type))
					<div class="location"><img loading="lazy" width="12"  src="{{ URL::asset('img/location-ico.png')}}" alt="icon"  />{{@$doc->address_1}}, @if(!empty($doc->locality_id)){{@$doc->locality_id['name']}},@endif
			   ...
			   <div class="fees">
				...
				@if(checkCountry() && in_array(1,$consult_type) || in_array(2,$consult_type))
					@if(in_array(1,$consult_type))
						<div class="cal-doctor"> <a onclick="showSlot({{$doc->id}}, '{{base64_encode(1)}}');" class="btn" href="javascript:void(0);"><img loading="lazy" width="12" height="12" src="{{ URL::asset('img/mobile-icon.png')}}" alt="icon"  />Tele Appointment</a> </div>
					@endif
					@if(in_array(2,$consult_type))
						<div class="cal-doctor clinical"> <a onclick="showSlot({{$doc->id}}, '{{base64_encode(2)}}');" class="btn in-clinic-btn" href="javascript:void(0);"><img loading="lazy" width="12" src="{{ URL::asset('img/in-clinic-icon.png')}}" alt="icon"  /> In-clinic Appointment</a> </div>
					@endif
				@endif
			</div>
            </div> 
        ...
        <div class="doc_available">
            @if($doc->available_now == 1)
            <p class="now"><img loading="lazy" width="15" height="15" src="{{ URL::asset('img/availablity-icon.png')}}" alt="icon"  />Available Today</p>
            @endif
          </div>
         <div class="rating_doctor-div"  title="{{@$rating_val}} Rating">
            <?php if(isset($rating_div)){ echo $rating_div; } ?>
        ...
        <ul>
          ...
          <li>
            <a data_id="{{$prime_doctor->id}}" info_name="{{@$prime_doctor->DoctorSlug->name_slug}}" href="{{route('findDoctorLocalityByType',[$cityName,$stype,@$prime_doctor->DoctorSlug->name_slug])}}" >
              <span class="doctor-img"><img loading="lazy" width="56" height="56" @if(@$prime_doctor->profile_pic != null) src="{{@$prime_doctor->profile_pic}}" @else src="{{ URL::asset('img/doc-img.png')}}" @endif alt="Dr. {{$prime_doctor->first_name.' '.$prime_doctor->last_name}}" /></span>
...
<script>
	...
	var lipos = $('ul li').length;
	
	i_a = 1;		
	i_sec = 1;		
	
	$("ul li a").each(function() {
	  $(this).attr("href", "#scroll-sctn-" + i_a++);
	});

	$("section").each(function() {
	  $(this).attr("id", "scroll-sctn-" + i_sec++);
	});
