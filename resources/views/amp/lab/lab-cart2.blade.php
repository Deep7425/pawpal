  ...
  <div class="container-fluid">
    <div class="container">
      <div class="tab-cart-wrapper">
        @if(!empty(@Session::get("CartPackages")))
        <div class="tabs-cart">
              ...
              <div class="order-overview">
                  ...
                  <div id="MyCartPage">
                      <div>
                          ...
                          <section>
                            <table cellpadding="0" cellspacing="0">
                              ...
                              @if(!empty(@Session::get("CartPackages")))
                              @foreach(@Session::get("CartPackages") as $package)
                              ...
                              <tr>
                                ...
                                ?>
                                <?php
                                 if ($package['type'] = "OFFER") {
                                   $type = "offer";
                                 }
                                 elseif ($package['type'] = "PROFILE") {
                                     $type = "profile";
                                 }
                                 else {
                                   $type = "offer";
                                ...
                                <td> <a href="{{route('LabDetails', ['id'=> base64_encode($package['name']), 'type' => base64_encode($type)])}}">{{$package['name']}} </a> </td>
                                <td>₹   @if($package['rate']['offer_rate'] != 'null') {{$package['rate']['offer_rate']}} @else {{$package['rate']['b2c']}}  @endif <span>₹ {{$package['rate']['b2c']}}</span></td>

                                <td align="center">
                                  <input type="hidden" name="" class="offerPrice" value="@if($package['rate']['offer_rate'] != 'null'){{$package['rate']['b2c']-$package['rate']['offer_rate']}}@else 0 @endif">
                            ...
                            <div class="patient-details">
                              ...
                              ?>
                                          ...
                                          <div class="radio-wrapper waitingTime top">
                                              ...
                                              <p><input id="gender1" type="radio" name="gender" id="test8" value="M" @if($user->gender == "Male") checked @endif><label for="gender1">Male</label></p>
                      ...
                      <p class="coupon-wrapper">
                                 ...
                                 <div class="delete-address">
                                   <a href="javascript:void(0)" data-id="{{$address->id}}" class="addressDeleteNow" click-delete="0"><i class="fa fa-trash" aria-hidden="true"></i></a>
                                 </div>
                                  <div class="float">
                                    <?php
                              ...
                              <div class="emptyAddress">
                                <h3>No Any Address Available !</h3>
                            ...
                            <div class="addAddressDiv waitingTime" style="display:none;">
                              <div id="addAddressForm" class="form-address-details">
                                <div class="input-wrapper">
                                  <label>Address<i class="required_star">*</i></label>
                                  <input type="text" placeholder="Address" class="inputvalidation" name="address" maxlength="100"  value="" />
                                  <span class="help-block"></span>
                                </div>
                                <div class="input-wrapper">
                                ...
                                <div class="input-wrapper">
                                  ...
                                  <input type="text" placeholder="Locality" name="locality" class="inputvalidation" maxlength="50" value="" />
                                  <span class="help-block"></span>
                                </div>
                                <div class="input-wrapper">
                                  <label>Landmark<i class="required_star">*</i></label>
                                  <input type="text" placeholder="Landmark" name="landmark" class="inputvalidation" maxlength="44" value="" />
                                  <span class="help-block"></span>
                                </div>
                                ...
                                <div class="radio-wrapper bottom">
                                  ...
                                  <p><input id="test7" type="radio" name="label_type" class="labelType" value="1" checked><label for="test7">Home</label></p>

                                  <p><input id="test8" type="radio" name="label_type" class="labelType" value="2"><label for="test8">Office</label></p>

                                  <p><input id="test9" type="radio" name="label_type" class="labelType" value="3"><label for="test9">Other</label></p>
                                  <span class="help-block"></span>
                                </div>
                ...
                <div class="right">
                  ...
                  <div class="right-block">
                    <input type="hidden" class="" id="totalAmount" name="total_amount" value="{{base64_encode($totalAmount)}}">
                    <input type="hidden" class="" id="priceDiscount" name="price_discount" value="{{base64_encode($totalDiscount  )}}">
                    ...
                    <input type="hidden" class="" id="coupanDiscountAmount" name="discount_amt" value="">
                    ...
                    <ul>
                      <li>M.R.P. Total  <div class="price-tag-wrap"><span>₹</span><span class="totalAmount">{{$totalAmount}}</span></div></li>
                      ...
                      <li class="total-bill">Total <div class="price-tag-wrap"> <span>₹</span><span class="paidAmount">{{$offerAmount}}</span></div></li>
                    </ul>
                    <div class="total-save">Total Savings <div class="total-price"><strong>₹</strong><strong class="totalSaving">{{$totalDiscount}}</strong></div></div>
...
function viewCart(reportType=null) {
  jQuery('.loading-all').show();
  jQuery.ajax({
  type: "POST",
  dataType : "JSON",
...
function billCalculate(currentRow) {
  ...
  //
  // if ($(".report_type:checked").val() == "Y") {
  //   var cost = parseInt($(".report_type:checked").attr('cost'));
  //   $('#paidAmount').val(paidAmount + cost);
  //   $('.paidAmount').text(paidAmount + cost);
  // }
  // else {
  //   var paidAmount2 = $('#paidAmount2').val();
  //   $('#paidAmount').val(parseInt(paidAmount2));
  //   $('.paidAmount').text(parseInt(paidAmount2));
  // }
  //
  // if (coupanDiscount != "" ){
  //   $('#paidAmount').val(paidAmount-coupanDiscount)
  //   $('.paidAmount').text(paidAmount-coupanDiscount)
  // }
}
...
function paymentCalculate(type, currentRow=null) {
  ...
  if (type == 1) {
    ...
    $('#totalAmount').val(btoa(totalAmount-thisPackagePrice));
    ...
    $('.totalAmount').text(totalAmount-thisPackagePrice);
...
function GetAppointmentSlots(pincode, schedule_date) {
  ...
  success: function(data){
      ...
      if (data.LSlotDataRes > '0') {
        ...
        jQuery("#scheduleTime").html('<option value="">Slots Not Available</option>');
      }
